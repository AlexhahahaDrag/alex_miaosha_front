<template>
  <div>
    <a-modal :open='props.open'
      ...
      <a-form ref='formRef' name='PrepaidConsumeRecordTForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        ...
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['cardId'].name" :label="labelMap['cardId'].label">
              <a-input v-model:value='formState.cardId' :placeholder="'请填写' + labelMap['cardId'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['orderNo'].name" :label="labelMap['orderNo'].label">
              <a-input v-model:value='formState.orderNo' :placeholder="'请填写' + labelMap['orderNo'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['amount'].name" :label="labelMap['amount'].label">
              <a-input v-model:value='formState.amount' :placeholder="'请填写' + labelMap['amount'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['balanceAfter'].name" :label="labelMap['balanceAfter'].label">
              <a-input v-model:value='formState.balanceAfter' :placeholder="'请填写' + labelMap['balanceAfter'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['merchantName'].name" :label="labelMap['merchantName'].label">
              <a-input v-model:value='formState.merchantName' :placeholder="'请填写' + labelMap['merchantName'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['consumeTime'].name" :label="labelMap['consumeTime'].label">
              <a-date-picker v-model:value="formState.consumeTime" :format="'YYYY-MM-DD'"
...
import { PrepaidConsumeRecordTDetail } from './prepaidConsumeRecordTDetailTs';
import {
  getPrepaidConsumeRecordTDetail,
  addOrEditPrepaidConsumeRecordT,
} from '@/api/selfFinance/prepaidConsumeRecordT/prepaidConsumeRecordTTs';
import { message, FormInstance } from 'ant-design-vue';
import { ModelInfo } from '../prepaidConsumeRecordTListTs';
...
const handleOk = (): void => {
  ...
  if (formRef.value) {
    formRef.value.validateFields().then(
      () => savePrepaidConsumeRecordTManager()).catch(() => {
        loading.value = false;
      });
  }
};
