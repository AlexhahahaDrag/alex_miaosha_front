<template>
  <div>
    <a-modal :open='props.open'
      ...
      <a-form ref='formRef' name='PrepaidCardInfoTForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        ...
        <a-row :gutter='24'>
          ...
          <a-col :span='12'>
            <a-form-item :name="labelMap['cardName'].name" :label="labelMap['cardName'].label">
              <a-input v-model:value='formState.cardName' :placeholder="'请填写' + labelMap['cardName'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['initialBalance'].name" :label="labelMap['initialBalance'].label">
              <a-input v-model:value='formState.initialBalance' :placeholder="'请填写' + labelMap['initialBalance'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['currentBalance'].name" :label="labelMap['currentBalance'].label">
              <a-input v-model:value='formState.currentBalance' :placeholder="'请填写' + labelMap['currentBalance'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['expireDate'].name" :label="labelMap['expireDate'].label">
              <a-datap v-model:value='formState.expireDate' :placeholder="'请填写' + labelMap['expireDate'].label"></a-datap>
              <a-date-picker
        ...
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['cardStatus'].name" :label="labelMap['cardStatus'].label">
              <a-input v-model:value='formState.cardStatus' :placeholder="'请填写' + labelMap['cardStatus'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
...
} from '@/api/selfFinance/prepaidCardInfoT/prepaidCardInfoTTs';
...
const handleOk = (): void => {
  ...
  if (formRef.value) {
    formRef.value.validateFields().then(
      () => savePrepaidCardInfoTManager()).catch(() => {
        loading.value = false;
      });
  }
};
