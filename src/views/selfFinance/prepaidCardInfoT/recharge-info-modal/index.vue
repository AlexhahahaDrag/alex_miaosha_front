<template>
  <!-- 充值对话框 -->
  <a-modal v-model:open="rechargeDialogOpen" title="充值" @ok="confirmRecharge" @cancel="cancelRecharge">
    <a-form :model="rechargeForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      <a-form-item label="卡名称">
        <a-input v-model:value="rechargeForm.cardName" style="width: 100%"
          :disabled="rechargeForm.type === 'consume'" />
      </a-form-item>
      <a-form-item :label="rechargeForm.type === 'recharge' ? '充值金额' : '消费金额'">
        <a-input-number v-model:value="rechargeForm.consumeAmount" :min="1" style="width: 100%" />
      </a-form-item>
      <a-form-item label="描述">
        <a-textarea v-model:value="rechargeForm.description" style="width: 100%" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
