<template>
  <div class="page-info">
          ...
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :name="labelMap['stockId'].name" :label="labelMap['stockId'].label">
                <a-input v-model:value="searchInfo.stockId" :placeholder="'请选择' + labelMap['stockId'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item :name="labelMap['attrCode'].name" :label="labelMap['attrCode'].label">
                <a-input v-model:value="searchInfo.attrCode" :placeholder="'请选择' + labelMap['attrCode'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
          ...
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :name="labelMap['attrValue'].name" :label="labelMap['attrValue'].label">
                <a-input v-model:value="searchInfo.attrValue" :placeholder="'请选择' + labelMap['attrValue'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
    ...
    <div class="button">
      <a-space>
        <a-button type="primary" @click="editShopStockAttrs('add')">新增</a-button>
        <a-button type="primary" @click="query">导入</a-button>
        <a-button type="primary" danger @click="batchDelShopStockAttrs">删除</a-button>
      </a-space>
    </div>
    <div class="content">
      <a-table :dataSource="dataSource" :columns="columns" :loading="loading" :row-key="(record) => record.id"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a-space>
              <a-button type="primary" size="small" @click="editShopStockAttrs('update', record.id)">编辑</a-button>
              <a-popconfirm title="确认删除?" ok-text="确认" cancel-text="取消" @confirm="delShopStockAttrs(record.id)"
                @cancel="cancel">
      ...
      <ShopStockAttrsDetail ref="editInfo" :open="visible" :modelInfo="modelInfo" @handleOk="handleOk"
        @handleCancel="handleCancel"></ShopStockAttrsDetail>
    </div>
  </div>
</template>
