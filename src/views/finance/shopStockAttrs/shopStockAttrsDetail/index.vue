<template>
  <div>
    <a-modal :open='props.open'
      ...
      <a-form ref='formRef' name='ShopStockAttrsForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        @finishFailed='onFinishFailed' :rules='rulesRef' :label-col='labelCol' :wrapper-col='wrapperCol'>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['stockId'].name" :label="labelMap['stockId'].label">
              <a-input v-model:value='formState.stockId' :placeholder="'请填写' + labelMap['stockId'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['attrCode'].name" :label="labelMap['attrCode'].label">
              <a-input v-model:value='formState.attrCode' :placeholder="'请填写' + labelMap['attrCode'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['attrName'].name" :label="labelMap['attrName'].label">
              <a-input v-model:value='formState.attrName' :placeholder="'请填写' + labelMap['attrName'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['attrValue'].name" :label="labelMap['attrValue'].label">
              <a-input v-model:value='formState.attrValue' :placeholder="'请填写' + labelMap['attrValue'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
...
import { ShopStockAttrsDetail } from './shopStockAttrsDetailTs';
import {
...
import { ModelInfo } from '../shopStockAttrsListTs';
...
const handleOk = (): void => {
  ...
  if (formRef.value) {
    formRef.value.validateFields().then(
      () => saveShopStockAttrsManager()).catch(() => {
        loading.value = false;
      });
  }
};
...
const init = (): void => {
  ...
  if (props.modelInfo) {
    if (props.modelInfo.id) {
      getShopStockAttrsDetail(props.modelInfo.id)
        .then((res) => {
          if (res.code == '200') {
