  ...
  <div class="page-info">
          ...
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :name="labelMap['shopId'].name" :label="labelMap['shopId'].label">
                <a-input v-model:value="searchInfo.shopId" :placeholder="'请选择' + labelMap['shopId'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
            ...
            <a-col :span="8">
              <a-form-item :name="labelMap['customerId'].name" :label="labelMap['customerId'].label">
                <a-input v-model:value="searchInfo.customerId" :placeholder="'请选择' + labelMap['customerId'].label" allow-clear />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row :gutter="24">
    ...
    <div class="button">
      <a-space>
        ...
        <a-button type="primary" danger @click="batchDelShopCart">删除</a-button>
      </a-space>
    </div>
    <div class="content">
      <a-table :dataSource="dataSource" :columns="columns" :loading="loading" :row-key="(record) => record.id"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a-space>
              <a-button type="primary" size="small" @click="editShopCart('update', record.id)">编辑</a-button>
              <a-popconfirm title="确认删除?" ok-text="确认" cancel-text="取消" @confirm="delShopCart(record.id)"
                @cancel="cancel">
      ...
      <ShopCartDetail ref="editInfo" :open="visible" :modelInfo="modelInfo" @handleOk="handleOk"
        @handleCancel="handleCancel"></ShopCartDetail>
    </div>
  </div>
</template>
...
import { getShopCartPage, deleteShopCart } from "@/api/finance/shopCart/shopCartTs";
