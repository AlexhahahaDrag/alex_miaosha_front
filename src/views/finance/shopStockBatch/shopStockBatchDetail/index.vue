<template>
  <div>
    <a-modal :open='props.open'
      ...
      <a-form ref='formRef' name='ShopStockBatchForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        ...
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['batchCode'].name" :label="labelMap['batchCode'].label">
              <a-input v-model:value='formState.batchCode' :placeholder="'请填写' + labelMap['batchCode'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
            <a-form-item :name="labelMap['batchName'].name" :label="labelMap['batchName'].label">
              <a-input v-model:value='formState.batchName' :placeholder="'请填写' + labelMap['batchName'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
...
const handleOk = (): void => {
  ...
  if (formRef.value) {
    formRef.value.validateFields().then(
      () => saveShopStockBatchManager()).catch(() => {
        loading.value = false;
      });
  }
};
