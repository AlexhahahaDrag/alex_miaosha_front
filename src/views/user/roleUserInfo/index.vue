<template>
  <div class="page-info">
    ...
    <div class="button">
      <a-space>
        <a-button type="primary" @click="editRoleUserInfo('add')">新增</a-button>
        <a-button type="primary" @click="query">导入</a-button>
        <a-button type="primary" danger @click="batchDelRoleUserInfo">删除</a-button>
      </a-space>
    </div>
    <div class="content">
      <a-table :dataSource="dataSource" :columns="columns" :loading="loading" :row-key="(record) => record.id"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a-space>
              <a-button type="primary" size="small" @click="editRoleUserInfo('update', record.id)">编辑</a-button>
              <a-popconfirm title="确认删除?" ok-text="确认" cancel-text="取消" @confirm="delRoleUserInfo(record.id)"
                @cancel="cancel">
      ...
      <RoleUserInfoDetail ref="editInfo" :visible="visible" :modelInfo="modelInfo" @handleOk="handleOk"
        @handleCancel="handleCancel"></RoleUserInfoDetail>
    </div>
  </div>
</template>
...
function query() {
  getRoleUserInfoListPage(searchInfo.value, pagination.value);
}

function handleTableChange(pagination) {
  getRoleUserInfoListPage(searchInfo.value, pagination);
}

function delRoleUserInfo(ids: string) {
  deleteRoleUserInfo(ids).then((res) => {
    if (res.code == "200") {
...
function getRoleUserInfoListPage(param: SearchInfo, cur: pageInfo) {
  loading.value = true;
  getRoleUserInfoPage(param, cur.current, cur.pageSize)
    .then((res) => {
      if (res.code == "200") {
...
//新增和修改弹窗
function editRoleUserInfo(type: string, id?: number) {
  if (type == "add") {
