          ...
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :name="labelMap['name'].name" :label="labelMap['name'].label">
                <a-input v-model:value="searchInfo.name" :placeholder="'请选择' + labelMap['name'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item :name="labelMap['path'].name" :label="labelMap['path'].label">
                <a-input v-model:value="searchInfo.path" :placeholder="'请选择' + labelMap['path'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item :name="labelMap['title'].name" :label="labelMap['title'].label">
                <a-input v-model:value="searchInfo.title" :placeholder="'请选择' + labelMap['title'].label" allow-clear />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item :name="labelMap['component'].name" :label="labelMap['component'].label">
                <a-input v-model:value="searchInfo.component" :placeholder="'请选择' + labelMap['component'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item :name="labelMap['redirect'].name" :label="labelMap['redirect'].label">
                <a-input v-model:value="searchInfo.redirect" :placeholder="'请选择' + labelMap['redirect'].label" allow-clear />
              </a-form-item>
            </a-col>
            <a-col :span="8">
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="8">
              <a-form-item :name="labelMap['orderBy'].name" :label="labelMap['orderBy'].label">
                <a-input v-model:value="searchInfo.orderBy" :placeholder="'请选择' + labelMap['orderBy'].label" allow-clear />
              </a-form-item>
            </a-col>
          </a-row>
             <a-row :gutter="24">
      ...
      <a-table :dataSource="dataSource" :columns="columns" :loading="loading" :row-key="(record) => record.id"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a-space>
              <a-button type="primary" size="small" @click="editMenuInfo('update', record.id)">编辑</a-button>
              <a-popconfirm title="确认删除?" ok-text="确认" cancel-text="取消" @confirm="delMenuInfo(record.id)"
                @cancel="cancel">
