<template>
  <div>
    <a-modal :visible='props.visible'
      ...
      <a-form ref='formRef' name='OrgUserInfoForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        ...
        <a-row :gutter='24'>
          <a-col :span='12'>
            <a-form-item :name="labelMap['orgId'].name" :label="labelMap['orgId'].label">
              <a-input v-model:value='formState.orgId' :placeholder="'请填写' + labelMap['orgId'].label"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span='12'>
...
import { OrgUserInfoDetail } from './orgUserInfoDetailTs';
import {
...
import { ModelInfo } from '../orgUserInfoListTs';
...
let formState = ref<OrgUserInfoDetail>({});
...
//保存用户公司信息表信息
function saveOrgUserInfoManager() {
  let method = '';
  ...
  addOrEditOrgUserInfo(method, formState.value)
    .then((res) => {
      if (res.code == '200') {
...
function init() {
  ...
  if (props.modelInfo) {
    if (props.modelInfo.id) {
      getOrgUserInfoDetail(props.modelInfo.id)
        .then((res) => {
          if (res.code == '200') {
