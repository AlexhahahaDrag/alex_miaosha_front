<template>
  <div>
    <a-modal :visible='props.visible'
      ...
      <a-form ref='formRef' name='RolePermissionInfoForm' class='ant-advanced-search-form' :model='formState' @finish='onFinish'
        ...
        <a-row :gutter='24'>
          ...
          <a-col :span='12'>
            <a-form-item :name="labelMap['permissionId'].name" :label="labelMap['permissionId'].label">
              <a-input v-model:value='formState.permissionId' :placeholder="'请填写' + labelMap['permissionId'].label"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter='24'>
...
import { RolePermissionInfoDetail } from './rolePermissionInfoDetailTs';
import {
...
import { ModelInfo } from '../rolePermissionInfoListTs';
...
let formState = ref<RolePermissionInfoDetail>({});
...
const handleOk = () => {
  ...
  if (formRef.value) {
    formRef.value.validateFields().then(
      () => saveRolePermissionInfoManager()).catch(() => {
        loading.value = false;
      });
  }
};
...
//保存角色权限信息表信息
function saveRolePermissionInfoManager() {
  let method = '';
  ...
  addOrEditRolePermissionInfo(method, formState.value)
    .then((res) => {
      if (res.code == '200') {
...
function init() {
  ...
  if (props.modelInfo) {
    if (props.modelInfo.id) {
      getRolePermissionInfoDetail(props.modelInfo.id)
        .then((res) => {
          if (res.code == '200') {
